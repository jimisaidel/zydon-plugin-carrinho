    ...
    const loadChartData = async () => {
      ...
      try {
        ...
        if (!response || !response.items || !Array.isArray(response.items)) {
          console.log("Dados inválidos para gráfico de usuários")
          setData([])
          return
        }
        ...
        const userCounts = carts.reduce((acc: Record<string, number>, cart: any) => {
          const userName = cart.user_name || 'Usuário sem nome'
        ...
        const formattedData = Object.entries(userCounts)
          .map(([userName, count]: [string, any]) => ({
            usuario: userName.length > 20 ? `${userName.substring(0, 20)}...` : userName,
            carrinhos: count,
          }))
          .sort((a: any, b: any) => b.carrinhos - a.carrinhos)
          .slice(0, 10) // Mostrar apenas os top 10 usuários
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 500 }}>
              Carrinhos por Usuário
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Quantidade de carrinhos por usuário (todos os status)
            </Typography>
          </Box>
